    ...
    for rotation_file in rotations:
        ...
        for p in preprocess:
            ...
            for k in kabsch:
                ...
                if(algorithm == 0):  
                    results_MTMM[subject, exercise, rotation_index, preprocess_index, kabsch_index] = MTMM.run(subject=subject, exercise=exercise, unit=unit, rotation_file=rotation_file_path, preprocess=p, kabsch=k)
                else:
                    results_MTW[subject, exercise, rotation_index, preprocess_index, kabsch_index]= MTW.run(subject=subject, exercise=exercise, unit=unit, rotation_file=rotation_file_path, preprocess=p, kabsch=k)
                kabsch_index += 1
