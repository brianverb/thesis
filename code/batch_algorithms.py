    ...
    for rotation_file in rotations:
        ...
        for k in kabsch:
            ...
            if(algorithm == 0):
                ...
                print(acc)
                print(conf)
                add_result_tuple(results_MTMM, subject, exercise, rotation_index, kabsch_index, conf, acc, amount_exercises)
            else:
                acc, conf, amount_exercises = MTW.run(subject=subject, exercise=exercise, unit=unit, rotation_file=rotation_file_path, kabsch=k)
                print(acc)
                print(conf)
                add_result_tuple(results_MTW,  subject, exercise, rotation_index, kabsch_index, conf, acc, amount_exercises)
            kabsch_index += 1
            
            
        rotation_index += 1

def add_result_tuple(results, subject, exercise, rotation_index, kabsch_index, conf, acc, amount_exercises):
    mp = 0
    mc = 0
    fp = 0
    correct = 0
    ...
    results[subject,exercise,rotation_index, kabsch_index] = (correct/total, mp/amount_exercises, fp/(total-mp), mc/(total-fp-mp), total-mp, amount_exercises)
